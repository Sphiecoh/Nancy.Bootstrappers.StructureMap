...
var xunit = "test/Nancy.Bootstrappers.StructureMap.Tests/bin/"+configuration+"/net452/unix-x64/dotnet-test-xunit.exe";
  ...
  .Does(() =>
{
  ...
  var file = new FilePath("src/Nancy.Bootstrappers.StructureMap/project.json");
  ...
  .Does(() =>
{
  ...
  else 
  {
    ...
    using(var process = StartAndReturnProcess("mono", new ProcessSettings{Arguments = 
      xunit + " " + 
      "test/Nancy.Bootstrappers.StructureMap.Tests/bin/"+configuration+"/net452/unix-x64/Nancy.Bootstrappers.StructureMap.Tests.dll"}))
    {
      process.WaitForExit();
...
Task("Package")
  ...
  .Does(() =>
{
  var package = outputPackages + File("Nancy.Bootstrappers.StructureMap-Latest.zip");
